...
本文说明如何在 macOS 上使用 VS Code 编写 HarmonyOS ArkTS 代码，并通过官方 Command Line Tools、`hvigorw` 和 `hdc` 在真机上构建、安装、启动和查看日志。
...
- macOS 终端，默认以 `zsh` 为例
- VS Code，用于编辑项目文件
- HarmonyOS 官方 Command Line Tools，与目标 HarmonyOS SDK 版本匹配
...
## 2. 下载 Command Line Tools 和 SDK

从华为开发者联盟的 HarmonyOS 下载中心下载适用于 macOS 的 **Command Line Tools**，并下载与项目匹配的 SDK。
...
# 将对应 SDK 解压到 $HOME/HarmonyOS/Sdk
```

目录不必和示例完全一致，但后续环境变量必须指向实际位置。
...
find "$HARMONY_SDK_HOME" -type f -name hdc
```
