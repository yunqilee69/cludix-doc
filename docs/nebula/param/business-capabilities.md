...
从当前代码与 README 可以确认，`nebula-param` 对业务系统提供的能力主要包括七类：
...
- 调整会员等级上限后，同步刷新依赖该阈值的缓存
- 关闭某个功能开关后，立刻停掉相关定时任务
- 删除非内建参数前，先检查业务侧是否仍在引用该 `paramKey`

这类“必须和参数写入同事务成功或失败”的逻辑，适合实现 `SystemParamOperationHook`。

如果只是通知其他模块“某个参数变了”，例如刷新本地缓存、写审计、异步重算，则应监听 `param-created` / `param-updated` / `param-deleted`，而不是在 Hook 里做跨模块副作用。

---

## 8. 写操作 Hook 能解决什么

业务实现一个 `SystemParamOperationHook` Bean 后，可以在参数创建、更新、按 key 保存、删除和批量改值时介入。

典型用途：

- 在 `beforeUpdateSystemParam` 中根据 `existing.getParamKey()` 判断关联业务是否允许本次变更
- 在 `beforeDeleteSystemParam` 中阻止仍被业务引用的参数被删掉
- 在 `afterSaveOrUpdateSystemParamByKey` 中按 `created` 区分“首次补齐”和“覆盖已有值”
- 在批量更新前后做一次模块级缓存失效或一致性检查

不适合放进 Hook 的事情：

- 跨服务通知、发邮件、调外部系统
- 依赖参数值的异步处理（事件负载本身不含 `paramValue`，需要值应事后按 key 读取）
...
- 其他模块不必轮询参数表，也能感知配置变化
- `param-updated` 带 `valueChanged`，监听器可以忽略“只改了名称/模块归属”的更新
- 载荷不含参数值，适合进入 outbox 和事件日志

---

## 10. 业务落地建议

### 10.1 什么配置适合放到 param 模块
