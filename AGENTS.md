...
**所有 sidebar 都手写，禁止使用 `type: 'autogenerated'`**。`nebula` 和 `projects` 曾经是自动生成的，已全部改为手动：自动生成按目录名字母序排列，既表达不了阅读顺序，也给不了分类中文名。
...
新增 sidebar 要在 `sidebars/index.ts` 注册；多文件板块用聚合文件（参考 `sidebars/projects/index.ts`、`sidebars/tutorials/index.ts`）先导出、再登记进 `sidebars` 对象。

### 3.2 新增文档必须同步改 sidebar

**新增、改名、移动、删除文档后，必须手动更新对应 sidebar 文件的条目**，否则文档不会出现在左侧导航里（只能靠 URL 直接访问）。

```typescript
// sidebars/tutorials/docker.ts 真实结构
const dockerSidebar: SidebarConfig = [
  { type: 'doc', id: 'tutorials/docker/index', label: 'Docker' },
  { type: 'doc', id: 'tutorials/docker/docker-config', label: 'Docker 配置' },
  {
    type: 'category',
    label: '容器部署',
    link: { type: 'doc', id: 'tutorials/docker/deployments/index' },
    items: [
      'tutorials/docker/deployments/MySQL部署', // 新增的部署文档追加在这里
...
条目顺序即**阅读顺序 / 使用顺序**，不要按字母序或文件名排。

### 3.3 Sidebar 配置注意事项

- 使用文档 ID（去掉 `docs/` 前缀和 `.md` 后缀）：`docs/a/b/index.md` → `a/b/index`，页面 URL 仍是 `/docs/a/b/`
...
```bash
npm run build          # onBrokenLinks: "throw"，sidebar 里写错的文档 ID 会直接构建失败
...
在对应的 sidebar 文件中添加文档配置项（**必须手动添加，禁止 `autogenerated`**，文件对应关系见 [3.1 节](#31-侧边栏文件与文档范围对应关系)）：
...
**只写可复用步骤，不写本机特例。** 教程正文只保留命令、配置项、失败现象和通用环境变量。调试过程中的本机细节（某台机器的 alias、某次实测数据、Agent 执行方式）不进文档。
